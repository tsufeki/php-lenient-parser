...
namespace PhpLenientParser\Unserializer;

use XMLReader;
use DomainException;
use PhpLenientParser\Unserializer;
...
class XML implements Unserializer
{
    ...
    protected function readComment() {
        $className = $this->reader->getAttribute('isDocComment') === 'true'
            ? 'PhpLenientParser\Comment\Doc'
            : 'PhpLenientParser\Comment'
        ;
    ...
    protected function getClassNameFromType($type) {
        $className = 'PhpLenientParser\\Node\\' . strtr($type, '_', '\\');
